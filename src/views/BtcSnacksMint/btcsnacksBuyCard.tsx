...
export default function SnacksBuyCard() {
  ...
  } = useSnacksBuyInfo(parsedTokenAmount, tokens.wbtc)
  ...
  const [approval, approveCallback] = useApproveCallback(requiredAmount, tokens.btcsnacks.address)
  ...
  const { btcsnacksBuy } = useBtcSnacksMint()
  const handleSwap = async () => {
    try {
      await btcsnacksBuy(parsedTokenAmount?.toExact())
    } catch (e) {
      console.log(e)
    }
  }
  ...
  return (
    <StyledSwapContainer $isChartExpanded={false}>
      <StyledInputCurrencyWrapper>
        <AppBody>
          <CurrencyInputHeader title={t('BtcSnacks Buy')} subtitle={t('Buy BtcSnacks in an instant')} />
                ...
                <RowBetween>
                  ...
                  >
                    {approval === ApprovalState.PENDING ? (
                      ...
                      t('Enable %asset%', { asset: tokens.wbtc.symbol ?? '' })
                    )}
                  </Button>
                  <Button
                    variant="primary"
          ...
          <LightCard>
            <Text fontSize="14px" style={{ textAlign: 'center' }}>
              ...
              {t('You can only mint WHOLE btcsnacks not partial. There are 5% mint fees')}
            </Text>
          </LightCard>
        </AppBody>
      </StyledInputCurrencyWrapper>
